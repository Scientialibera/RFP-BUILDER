...
import { AlertCircle, CheckCircle, Download, Loader, MessageSquare, Upload, Wand2 } from 'lucide-react';
import { FileUpload } from './FileUpload';
import { exportRequirementsToExcel, importRequirementsFromExcel } from '../utils/requirementsExcel';
import {
  critiqueRFP,
  extractReqs,
  generateCustomRFP,
...
export function CustomFlow({
  ...
  useEffect(() => {
    ...
    const loadRunIntoFlow = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(getRequestErrorMessage(err, 'Failed to load run into Custom Flow'));
        }
      } finally {
        if (!cancelled) {
          setLoadingRun(false);
  ...
  const categoryOptions = useMemo(() => {
    ...
    for (const req of analysis?.requirements ?? []) {
      const value = req.category?.trim();
      if (!value) continue;
      if (baseSet.has(value.toLowerCase())) continue;
      extras.add(value);
    }
    return [...base, ...Array.from(extras).sort((a, b) => a.localeCompare(b))];
  }, [analysis]);
  ...
  const handleExtractReqs = async () => {
    ...
    try {
      ...
      setError(getRequestErrorMessage(err, 'Failed to extract requirements'));
    } finally {
      setLoadingStep(null);
    }
  };
  ...
  const handlePlan = async () => {
    ...
    try {
      ...
      setError(getRequestErrorMessage(err, 'Failed to generate plan'));
    } finally {
      setLoadingStep(null);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(getRequestErrorMessage(err, 'Failed to generate RFP response'));
    } finally {
      setLoadingStep(null);
    }
  };
  ...
  const handleCritique = async () => {
    ...
    try {
      ...
      setError(getRequestErrorMessage(err, 'Failed to critique RFP'));
    } finally {
      setLoadingStep(null);
    }
  };
  ...
  const handleExportRequirements = () => {
    ...
    exportRequirementsToExcel(analysis.requirements);
  };
  ...
  const handleImportRequirements = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportMessage(null);
      setImportFile(file);
      const result = await importRequirementsFromExcel(file);
      const nextAnalysis: RFPAnalysis = analysis
        ? { ...analysis, requirements: result.requirements }
        : { summary: 'Imported from Excel', requirements: result.requirements };
      setAnalysis(nextAnalysis);
      appendAnalysisVersion(nextAnalysis, `Imported from ${result.filename}`);
      setImportMessage(
        `Imported ${result.requirements.length} requirements from "${result.filename}" (${result.totalRows} rows read)`
      );
    } catch (err) {
      setError(getRequestErrorMessage(err, 'Failed to import requirements'));
    }
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        {importFile && (
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-600">
            <div className="inline-flex items-center gap-2 border border-gray-200 rounded-full px-3 py-1 bg-gray-50">
              <span className="font-medium text-gray-700">Imported file</span>
              <span className="text-gray-500">{importFile.name}</span>
            </div>
            <button
              type="button"
